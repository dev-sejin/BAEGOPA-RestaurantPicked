...
final class RestaurantDetailViewController: UIViewController {
    ...
    private func updateMapView() {
        let lon = String(myLocation.coordinate.longitude)  // 경도 정보
        let lat = String(myLocation.coordinate.latitude)  // 위도 정보
        let coord = "\(lon),\(lat)"
        print(coord)
        ...
        ReverseGeocodingAPI.shared.getDataFromAPI(coord: coord) { [unowned self] data in
            ...
            self.webViewURL += "\(street)" // webViewURL 뒤에 주소(동) 추가 (검색 정확도 또는 이름이 같은 식당이 있을 경우를 대비해서)
            print(string)
